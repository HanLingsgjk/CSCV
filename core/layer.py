...
class GRN(nn.Module):
    ...
    def __init__(self, dim):
        ...
        self.xita = nn.Parameter(torch.zeros(1, 1, 1, dim))#后面那个能不能直接用这个解决阿

    def forward(self, x):
        Gx = torch.norm(x, p=2, dim=(1,2), keepdim=True)
        Nx = Gx / (Gx.mean(dim=-1, keepdim=True) + 1e-6)
        return self.gamma * (x * Nx) + self.beta + x
class GRNCV(nn.Module):
    """ GRNCV (Global Response Normalization) layer
    """
    def __init__(self, dim):
        super().__init__()
        self.gamma = nn.Parameter(torch.zeros(1, dim, 1, 1))
        self.beta = nn.Parameter(torch.zeros(1, dim, 1, 1))
        #self.xita = nn.Parameter(torch.zeros(1, dim, 1, 1))#比例x后面看看要不要学
...
class ConvNeXtV2tiny(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.final(input + x)
        return x
class ConvNeXtCorr(nn.Module):

    def __init__(self, dim, output_dim,bl = 4,kszie = 3):
        super().__init__()
        self.dwconv = nn.Conv2d(dim, dim, kernel_size=kszie, padding=0, groups=dim)  # depthwise conv
        self.norm = LayerNorm(dim, eps=1e-6)
        self.pwconv1 = nn.Linear(dim, bl * dim)  # pointwise/1x1 convs, implemented with linear layers
        self.act = nn.GELU()
        self.pwconv2 = nn.Linear(bl * dim, dim)
        self.final = nn.Conv2d(dim, output_dim, kernel_size=3, padding=0)
