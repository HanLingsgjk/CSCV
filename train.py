...
def sequence_loss(flow_preds,dc_preds, flow_gt,dc_gt, valid, gamma=0.8, max_flow=MAX_FLOW):
    ...
    for i in range(n_predictions):
        ...
        d_loss = (dc_preds[i].log() - gt_dc.log()).abs()
        flow_loss += i_weight * (valid[:, None] * i_loss).mean()
        dc_loss   += i_weight * (mask_dc[:, None] * d_loss).mean()

    d_loss = (dc_preds[-1].log() - gt_dc.log()).abs()
    dc_loss += i_weight * (mask_dc[:, None] * d_loss).mean()
    ...
    metrics = {
        'epe': epe.mean().item(),
        'ede': ede.mean().item()*10,
        'ede2': ede2.mean().item()*10,
...
class Logger:
    ...
    def _print_training_status(self):
        ...
        data = open("/home/xuxian/RAFT3D/ans/records.txt", 'a')
...
def train(args):
    ...
    model = nn.DataParallel(RAFT343used(args), device_ids=args.gpus)
    ...
    while should_keep_training:
        ...
        for i_batch, data_blob in enumerate(train_loader):
            ...
            flow_predictions,dc_predictions = model(image1, image2,iters=12)

            loss1,loss2, metrics = sequence_loss(flow_predictions,dc_predictions, flow,dc_change, valid, args.gamma)
            ...
            if total_steps % VAL_FREQ == VAL_FREQ - 1:
                ...
                for val_dataset in args.validation:
                    if val_dataset == 'chairs':
                        ...
                        results.update(evaluate.validate_kitti_test(model.module))
